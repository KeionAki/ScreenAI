...
- **三个快捷键**：「分析」截图并解答；「开始键入代码」把上一次分析得到的代码逐字键入到光标处（见下一节）；「停止键入」随时中断。默认分别是 ⌘⇧A、⌘⇧D、⌘⇧. ，单键模式下为 F5、F6、F8。
...
编程题分两步：先按「分析」解答，代码会被自动提取并暂存，字幕显示「已完成」；再由你按「开始键入代码」把它逐字键入到当前光标处。键入不会自动开始。
...
1. 按「分析」快捷键解答编程题，字幕显示「已完成」表示代码已准备好，菜单栏会显示「键入代码到光标 · N 行」。
2. 点进编辑器，把光标放到要插入的位置。
3. 按「开始键入代码」快捷键，倒计时结束后开始逐字键入。
4. 需要中断时按「停止键入」。

暂存的代码保留到下一次分析为止；如果下一次分析的结果不是编程题，暂存会被清空，菜单显示「暂无代码」。

**可调参数**（设置 › 捕获设置 › 键入到光标）：键入速度（3–80 字符/秒，默认 25）、速度抖动（默认 30%，让节奏更自然）、开始前倒计时（默认 2 秒）、制表符展开空格数。面板上的「测试键入」可以先在编辑器里试一段示例代码。

**在全屏的浏览器里使用：** 先点一下编辑器把光标放好，再按「开始键入代码」。这本来就是正常流程；但若网页刚进入全屏后一次都没点过，浏览器会丢弃开头的一部分输入，表现为第一行没打出来。另外应用默认用「移动光标」而不是按 Esc 来关闭补全浮层，因为 Esc 会让网页退出全屏。

**安全设计：** 焦点在 ScreenAI 自己的窗口上时会拒绝键入；键入过程中会暂停截图捕获，快捷键和定时捕获都不会触发新的分析，避免覆盖正在键入的代码；可随时用快捷键或菜单栏停止；只键入代码块内容，模型的解释文字不会被打进去。

**已在真实环境中验证**：VSCode 里 25 行 574 字符的代码键入后与模型输出逐字节一致；全屏的 Chrome 网页编辑器里内容同样完整，且不会退出全屏。
